...
if [[ ! -e "$shlogTmpDir" ]]; then
        mkdir -p "$shlogTmpDir"
        if [[ $? -ne 0 ]]; then
                echo "ERROR! - LOGDIR ($shlogTmpDir) does not exist and can not be created. Exiting..."
                exit 1
        fi
elif [[ ! -d "$shlogTmpDir" ]]; then
        echo "ERROR! - LOGDIR ($shlogTmpDir) exists but is not a directory. Exiting..."
        exit 1
elif [[ ! -w "$shlogTmpDir" ]]; then
        echo "ERROR! - LOGDIR ($shlogTmpDir) exists but is not writable. Exiting..."
        exit 1
fi

# Check if dir has a leading slash and add it if it doesnt
if [[ ! "$shlogTmpDir" =~ /$ ]]; then
        shlogTmpDir="$shlogTmpDir"/
fi
}
...
if [[ -n "$shlogTmpPath" ]]; then
        ...
        # Input is a dir, append default filename
                ...
                # TODO: Validate shlogTmpPath (as dir)

                # Populate $LOGFILE
                shlog_logfile

                LOGPATH="$shlogTmpDir$LOGFILE"

        # Input is a filename, append default dir
        elif [[ "$shlogTmpPath" =~ [^/]+\.[^/]{1,3} ]]; then
                # Populate $LOGDIR
                shlog_logdir
                # TODO: Validate $LOGDIR
